...
// retrieve filtered and unfiltered product options
  ...
  if (isset($unfilteredManufacturers) && count((array)$unfilteredManufacturers) > 1)
  {
    $unfilteredManufacturers = array_filter(array_unique($unfilteredManufacturers));
  }
  if (isset($unfilteredCategories) && count((array)$unfilteredCategories) > 1)
  {
    $unfilteredCategories = array_filter(array_unique($unfilteredCategories));
  }
  if (isset($unfilteredProducts) && count((array)$unfilteredProducts) > 1)
  {
    $unfilteredProducts = array_filter(array_unique($unfilteredProducts));
  }
  if (isset($filteredManufacturers) && count((array)$filteredManufacturers) > 1)
  {
    $filteredManufacturers = array_filter(array_unique($filteredManufacturers));
  }
  if (isset($filteredCategories) && count((array)$filteredCategories) > 1)
  {
    $filteredCategories = array_filter(array_unique($filteredCategories));
  }
  if (isset($filteredProducts) && count((array)$filteredProducts) > 1)
  {
    $filteredProducts = array_filter(array_unique($filteredProducts));
  }
  if (isset($priceArray) && count((array)$priceArray) > 1)
  {
 ...
 */
 ...
 {
  if (isset($priceArray) && count((array)$priceArray) > 0)
  {
 ...
 */
  ...
  if(isset($filteredProducts) && count((array)$filteredProducts) > 0)
    {
...
// BOF language fix by a_berezin
    $attributes = $db->Execute("SELECT count(DISTINCT p2a.products_id) as quantity, po.products_options_name, pov.products_options_values_name," .
         ...
         " JOIN " . TABLE_PRODUCTS_OPTIONS . " po ON p2a.options_id = po.products_options_id AND po.products_options_type=0 AND po.language_id=" . (int)$_SESSION['languages_id'] .
...
<button type="button" class="btn_filter-close-top" style="display: none;" onclick="$(\'#navColumnOneWrapper\').removeClass(\'panded\'); $(\'.button_filter-close\').removeClass(\'panded\'); $(\'.search-mobile--wrapper\').removeClass(\'panded\');">×</button>
  <div class="button_filter-close" onclick="$(\'#navColumnOneWrapper\').removeClass(\'panded\'); $(\'.button_filter-close\').removeClass(\'panded\'); $(\'.search-mobile--wrapper\').removeClass(\'panded\');">Back</div>';
?>
